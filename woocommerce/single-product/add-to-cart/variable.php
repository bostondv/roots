...
<form class="variations_form cart form-horizontal" method="post" enctype='multipart/form-data' data-product_id="<?php echo $post->ID; ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
	...
	<?php if ( ! empty( $available_variations ) ) : ?>
		...
		<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
			<div class="form-group">
				...
				<div class="col-sm-9">
				<select id="<?php echo esc_attr( sanitize_title($name) ); ?>" class="form-control" name="attribute_<?php echo sanitize_title($name); ?>">
			...
			<?php
				if ( sizeof( $attributes ) == $loop )
						echo '<div class="form-group"><div class="col-sm-9 col-sm-offset-3"><a class="reset_variations" href="#reset">' . __( 'Clear selection', 'woocommerce' ) . '</a></div></div>';
			?>
	    <?php endforeach;?>
		</div>
	...
	<?php else : ?>
		...
		<p class="alert alert-warning"><?php _e( 'This product is currently out of stock and unavailable.', 'woocommerce'); ?></p>
